    ...
    useEffect(() => {
        const getPost = async () => {
            try {
                ...
                if (data) {
                    setFormData({ title: data.title, desc: data.desc, category: data.category._id, file: data.file?._id });
                } else {
                ...
                console.error('Get post error:', error); // Log the error for debugging
            }
        };
        getPost();
    }, [postId]);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            let input = {
                ...
                file: fileId || formData.file
            };

            // Only include fileId if a new file has been uploaded
            if (fileId && fileId !== "") {
                input.file = fileId;  // Add fileId only if valid
            }

            console.log("Data being sent to API:", input);

            const response = await axios.put(`/post/${postId}`, input);
            toast.success(response.data.message);
            setFormData(initialFormData);  // Reset form
            setFormError(initialFormError);  // Clear any errors
            setLoading(false);
            navigate("/post");
        } catch (error) {
            setLoading(false);
